...
$(document).ready(function(){
    ...
    $(window).keydown(function(event){
      if(event.which == 13){
          var message_data = $("textarea").val();
          if ($.trim(message_data).length > 0){
              var message_info = {
    ...
    $.ajax({
        ...
        success: function (callback) {
            ...
            $.each(JSON.parse(callback)['contact_list'],function(k,v){
                CONTACT[v.id] = v.name;
                html +='<li style="margin-top:5px" onclick="ContactActive(this);" contact_id='+ v.id+' class="list-group-item">'+ v.name+'<span class="badge">10</span>'+'</li>'
            });
